...
These actions have their inputs defined with their specific properties and types.
  ...
  ["build"] {
    ...
    steps {
      (catalog.`actions/checkout@v6`) {
        with {
          // This is type-checked and will fail if the value is not an integer.
          `fetch-depth` = 0
        }
      }
      ...
      // You can use other actions too (untyped steps).
      // In this case, the inputs is simply typed as `Mapping<String, String | Number | Boolean>`.
      new {
